...
- **C4-C5** (API de chatbot, seleção de LLM) — endpoint `/diagnostico` entregue nesta data; removido em 2026-09-12, reimplementação planejada para trilha futura
- **C8** (API de diagnóstico de falhas) — endpoint principal desta entrega; removido em 2026-09-12, reimplementação planejada para trilha futura
- **C13** (documentação Swagger/OpenAPI) — Swagger automático do FastAPI em `/docs`, junto com a API removida acima
...
- **G1-G4** (classificação de falhas, detecção de anomalias em séries temporais, RCA assistida por LLM) — arquitetura híbrida de 3 camadas (pipeline 2 + API) entregue nesta data; API removida em 2026-09-12, reimplementação planejada para trilha futura
- **G9** (chatbot com diagnóstico) — chatbots especializados do dashboard

## Não coberto ainda (fica para as próximas trilhas)
- **A17, C14-C16** (MCP) — servidor MCP chegou a ser implementado após esta entrega, mas foi removido em 2026-09-12; reimplementação planejada para trilha futura
