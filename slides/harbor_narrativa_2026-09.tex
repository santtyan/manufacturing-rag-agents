...
\begin{frame}{Ato 6 --- O número que de fato importa}
  \small
  \begin{numerobloco}{Benchmark oficial, não proxy de retrieval}
    k-NN label purity é um proxy. A pergunta certa é: classifica a operação
    melhor que os modelos supervisionados publicados no benchmark oficial do
    dataset (\emph{openpack-torch}, split ``Pilot Challenge'')?
    \textbf{F1-macro = 0,91--0,92} (dois protocolos, amostra e teste completo
    de 2.591 janelas) contra UNet=0,3451, ST-GCN=0,7024, DeepConvLSTM=0,7081
    --- superando os três, \textbf{sem treinar nenhum modelo}.
  \end{numerobloco}

  \pause
  Custo, reportado junto: indexação ~50--72s, busca ~0,5s/janela
  (\emph{Multimodal-RAG-Survey}, ACL 2025, arXiv:2502.08826, confirma que isso
  é padrão de literatura, não só regra do Harbor).
\end{frame}
  ...
  \begin{numerobloco}{Um conjunto de decisões, três respostas diferentes}
    \begin{itemize}
      \item \textbf{Sim:} migração do self-repair e do roteador para LangGraph;
        ...
        OpenPack como 8º dataset no chat e no Streamlit, sem regressão
        (roteamento 55/67 $\to$ 59/69), F1-macro 0,91--0,92 acima do benchmark oficial.
